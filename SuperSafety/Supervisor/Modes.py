...
#                 new_state = run_dynamics_update(state, qact, self.time_step)
...
def test_modes():
    ...
    print(f"Modes: {m.qs}")
    print(f"Modes: {m.qs.shape}")
    
    l = m.qs.copy()
    for q in l:
        print(f"{q} -> {m.get_mode_id(q[0])}")
