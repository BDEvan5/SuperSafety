...
def build_viability_dynamics(phis, m, time, conf):
    ...
    dynamics = np.zeros((len(m), len(m), 1, 4), dtype=np.int)
    invalid_counter = 0
    for j, state_mode in enumerate(m.qs): # searches through old q's
        ...
        for k, action in enumerate(m.qs): # searches through actions
            ...
            if new_q is None:
                invalid_counter += 1
                dynamics[j, k, :] = np.nan # denotes invalid transition
                print(f"Invalid dyns: phi_ind: s_mode:{j}, action_mode:{k}")
                continue
            ...
            dynamics[j, k, 0, 2] = min(max(0, new_k), len(phis)-1)
            
            dynamics[j, k, 0, 0] = int(round(dx * resolution))                  
            dynamics[j, k, 0, 1] = int(round(dy * resolution))                  
            dynamics[j, k, 0, 3] = int(new_q) 
...
def build_dynamics_table(sim_conf):
    ...
    for i in range(dynamics.shape[0]): 
        print(f"{i}:: s: {m.qs[i]} --> {dynamics[i, :, 0, -1]}")
