...
def train_baseline_vehicle(env, vehicle, conf, show=False):
    ...
    for n in range(conf.train_n):
        ...
        if done or ep_steps > conf.max_steps:
            ...
            print(f"{n}::Lap done {lap_counter} -> LapTime {env.lap_times[0]:.2f} -> TotalReward: {vehicle.t_his.rewards[vehicle.t_his.ptr-1]:.2f}")
            lap_counter += 1
            ep_steps = 0 
            if s_prime['collisions'][0] == 1: crash_counter += 1
...
def train_kernel_vehicle(env, vehicle, conf, show=False):
    ...
    for n in range(conf.train_n):
        ...
        if s_prime['collisions'][0] == 1:
            print(f"COLLISION:: Lap done {lap_counter} -> {env.lap_times[0]} -> Inters: {vehicle.ep_interventions}-> Reward: {vehicle.planner.t_his.rewards[vehicle.planner.t_his.ptr-1]:.2f}")
            state, step_reward, done, info = env.reset(np.array([[conf.sx, conf.sy, conf.stheta]]))
            lap_counter += 1
